<template>
  <div class="container-table-cart pos-relative">
    <div class="wrap-table-shopping-cart bgwhite">
      <h1 class="mt-4 mb-4" style="text-align:center;">Detail</h1>
      ...
      <div class="contianer mt-4 mb-4 addnoticediv" v-if="checklogin()">
          <div class="header-cart-wrapbtn addnoticedivright">
            <button type="button" @click="show_detail(notice.idx)" class="flex-c-m size1 bg1 bo-rad-20 hov1 s-text1 trans-0-4 addnotice">수정하기</button>
          </div>
          <div class="header-cart-wrapbtn addnoticedivleft">
            <button type="button" @click="delete_notice(notice.idx)" class="flex-c-m size1 bg1 bo-rad-20 hov1 s-text1 trans-0-4 addnotice">삭제하기</button>
          </div>
      </div>
    </div>
  </div>
...
export default {
  ...
  methods: {
    ...
    checklogin() {
      return this.notice.username === this.loginmember.username || this.loginmember.userid === 'admin';
    }
  },
  mounted() {
    this.show_init();
